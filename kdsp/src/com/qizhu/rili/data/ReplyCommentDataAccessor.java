...
public class ReplyCommentDataAccessor extends AbstractDataAccessor<Chat> {

    @Override
    protected void getDataFromServer(final OnDataGetListener<Chat> listener, final boolean useAppend) {
        KDSPApiController.getInstance().findChatMsgList(getPage(useAppend), 10, new KDSPHttpCallBack() {
            @Override
